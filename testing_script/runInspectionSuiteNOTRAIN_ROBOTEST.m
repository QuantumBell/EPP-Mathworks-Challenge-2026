%NOTE: This script is different than runInspectionSuiteTRAIN since this
% script utilizes the testing set and network structure that is in
% testSetoftestingScriptNetworksAndClasses and
% testingScriptNetworksAndClasses, respectively. Thus, there is no code
% that will split the original data set table and train the network (the
% script runInspectionSuiteTRAIN has the code for that process). The split
% is random, and the network does learn on some images better than others,
% so different runs will have different accuracy and lose

% Script that is used to evaluate a set of models for image inspection
% This script is to be used on the latest modified dataset version. It
% utilizes an already trained set of networks and the test split from it.
% Evaluation will be performed through testing the networks on set of 
% images through the master function "classifyImg," which loops through all
% images in the test set, returning their label and confidence score. A 
% confusion matrix for all tested images for ALL NETWORKS will be 
% displayed, along with metrics of network accuracy, recall, and precision.
% Separate plots and tables containing yield and defect counts will also be
% outputted. Finally, a montage of images that are deemed as common 
% failures will also be outputted.

%--------WARNING: TASK 1 FROM LIVE SCRIPT MUST BE RUN BEFORE USING THIS
% SCRIPT, OTHERWISE, IT WILL NOT WORK

%The only difference between this script and runInspectionSuiteNOTRAIN.m is
% that the test set is modified to apply distortions to all of the images
% and test the classifiers on their robustness and how they perform under
% simulated variations, and that accuracy and false reject rates are compared.

%ALMOST IDENTICAL TO runInspectionSuiteNOTRAIN.m

%Testing script: NO TRAINING IS INVOLVED (run "runInspectionSuiteTRAIN") to
% split the dataset into training and testing sets and train your own
% networks for use in evaluating your system

%------WARNING: MAKE SURE TO LOAD testingScriptNetworksAndClasses.m and the
%test, training, and validation sets of testingScriptNetworksAndClasses.m 
% (they start with the name of the set, followed by
% testingScriptNetworksAndClasses (EX: 
% ______testingScriptNetworksAndClasses) before running this script 
% (they should be pre-loaded thanks to the live script)

clc;

%========================================

%Apply distortions to all images in the test set

%Create temporary folder to be used for applying distortions
robustFolder = fullfile(tempdir,"RobustnessTestImages");

if ~exist(robustFolder,"dir")
    mkdir(robustFolder);
end

%Create copy of test set to modify the images
robustTestSet = testSet;

%Add robustness information columns (these will store the parameters of the
% changed images (such as how much they were adjusted due to noise,
% brightness, blur, etc., whether or not a certain distortion was applied,
% along with the file name of each image to associate it correctly))

numImages = height(robustTestSet);

robustTestSet.NoiseApplied = false(numImages,1);
robustTestSet.NoiseVariance = zeros(numImages,1);

robustTestSet.BrightnessApplied = false(numImages,1);
robustTestSet.BrightnessShift = zeros(numImages,1);

robustTestSet.ContrastApplied = false(numImages,1);
robustTestSet.ContrastFactor = ones(numImages,1);

robustTestSet.BlurApplied = false(numImages,1);
robustTestSet.BlurSigma = zeros(numImages,1);

%Loop through all images and apply distortions
for k = 1:height(robustTestSet)

    I = imread(robustTestSet.Filename(k));

    %Apply noise to 70% of images
    if rand < 0.7

        noiseVariance = 0.0005 + rand()*0.01;

        I = imnoise(I,"gaussian",0,noiseVariance);

        robustTestSet.NoiseApplied(k) = true;
        robustTestSet.NoiseVariance(k) = noiseVariance;

    end

    %Apply random brightness to 70% of images
    if rand < 0.7

        brightnessShift = randi([-35 35]);

        I = imadd(I,brightnessShift);

        robustTestSet.BrightnessApplied(k) = true;
        robustTestSet.BrightnessShift(k) = brightnessShift;

    end

    %Apply random contrast to 70% of images
    if rand < 0.7

        contrastFactor = 0.7 + rand()*0.6;

        I = im2double(I);
        I = imadjust(I,[],[],contrastFactor);
        I = im2uint8(I);

        robustTestSet.ContrastApplied(k) = true;
        robustTestSet.ContrastFactor(k) = contrastFactor;

    end

    %Apply random blur to 70% of images
    if rand < 0.7

        blurSigma = rand()*2;

        I = imgaussfilt(I,blurSigma);

        robustTestSet.BlurApplied(k) = true;
        robustTestSet.BlurSigma(k) = blurSigma;

    end

    %Save newly edit image
    [~,name,ext] = fileparts(robustTestSet.Filename(k));
    newFilename = fullfile(robustFolder,name + "_robust" + ext);
    imwrite(I,newFilename);

    %Update filename in table to find the new image
    robustTestSet.Filename(k) = newFilename;
...
numTestImages = height(robustTestSet);

%Obtain Ground truth labels from test set for all images and store in
% vector
truePart = robustTestSet.Part;
trueCondition = robustTestSet.("Part Condition");
trueDefect = robustTestSet.("Defect Type");
...
resultsTable = table(robustTestSet.Filename, truePart, predictedPart,...
